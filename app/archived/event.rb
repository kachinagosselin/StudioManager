class Event < ActiveRecord::Base
    ...
    attr_accessible :description, :end_at, :instructor_id, :start_at, :title, :registered_events, :registered_events_attributes, 
    :archive, :price, :resource_type, :resource_id, :custom_url, :canceled, :location_id, :address, :start_on, :every, :end_on, :type
    ...
    scope :upcoming, not_canceled.where("start_at >= ?", Time.now - 15.minutes).order('events.start_at ASC')
    scope :sorted, order('events.created_at ASC')
    ...
    def attendance_descriptor
        if self.registered.count == 0 
            ...
            return "#{self.attendees.count} of #{self.registered.count}"
        end
    end
